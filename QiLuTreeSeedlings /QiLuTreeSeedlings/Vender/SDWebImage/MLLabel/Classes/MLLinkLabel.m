...
- (void)commonInit
{
    ...
    //默认除了话题和@都检测
//    MLDataDetectorTypePhoneNumber|
    _dataDetectorTypes = MLDataDetectorTypeURL|MLDataDetectorTypeEmail|MLDataDetectorTypeAttributedLink;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (!self.activeLink) {
        ...
        if ([self.delegate respondsToSelector:@selector(didtouchblankAreaAtion:)]) {
            [self.delegate didtouchblankAreaAtion:self.tag];
        }
    }
}
