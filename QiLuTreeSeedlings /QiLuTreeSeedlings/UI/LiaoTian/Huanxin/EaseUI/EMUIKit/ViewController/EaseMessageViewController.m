...
//            _conversation.ext=@{@"title":@""};
        }
        if (conversationType == EMConversationTypeGroupChat) {
            EMError *error = nil;
            EMGroup *group=[[EMClient sharedClient].groupManager fetchGroupInfo:conversationChatter includeMembersList:NO error:&error];
            if (!error) {
              _conversation.ext=@{@"title":group.subject};
            }
        }
...
//    }
    ...
    NSString *avatar=APPDELEGATE.userModel.headUrl;
    NSString *phone=APPDELEGATE.userModel.phone;
    NSString *identity=APPDELEGATE.userModel.goldsupplier;
    NSDictionary *ext =  @{@"avatar":avatar,@"nick":nick,@"phone":phone,@"identity":identity};
    [self sendTextMessage:text withExt:ext];
}
...
-(void)callAction:(NSMutableArray *)phoneAry
{
    if(phoneAry.count==0)
    {
        return;
    ...
    {
        __weak typeof(self)weakSelf=self;
        NSString *phoneNum=[phoneAry firstObject];
        NSString *alertStr=[NSString stringWithFormat:@"%@疑似一个手机号,您确定要拨打？",phoneNum];
        ...
        UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            [weakSelf callPhoneWithPhoneNum:[phoneAry firstObject]];
        }];
    ...
    {
        __weak typeof(self)weakSelf=self;
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"提示"message:@"拨打手机号"preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil];
        
        for (int i=0; i<phoneAry.count; i++) {
            NSString *phoneNum=phoneAry[i];
            UIAlertAction *okAction = [UIAlertAction actionWithTitle:phoneNum style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                [weakSelf callPhoneWithPhoneNum:phoneAry[i]];
            }];
            [alert addAction:okAction];
        }
...
-(void)callPhoneWithPhoneNum:(NSString *)num
{
