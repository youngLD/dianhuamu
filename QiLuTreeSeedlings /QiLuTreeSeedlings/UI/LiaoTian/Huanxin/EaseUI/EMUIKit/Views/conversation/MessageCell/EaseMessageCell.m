...
//    cell.messageTextColor = [UIColor blackColor];
    ...
    cell.phoneAry=[NSMutableArray array];
}
    ...
    if (self) {
        ...
        [self _setupSubviewsWithType:_messageType
                            isSender:model.isSender
                               model:model];
        self.phoneAry=[NSMutableArray array];
...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        switch (model.bodyType) {
            case EMMessageBodyTypeText:
            {
                NSMutableArray *ary=[self getPhoneNumber:model.text];
                
                NSMutableAttributedString *atString=[[EaseEmotionEscape sharedInstance] attStringFromTextForChatting:model.text textFont:self.messageTextFont];
                [self.phoneAry removeAllObjects];
                if (ary.count>0) {
                    for (int i=0; i<ary.count; i++) {
                        NSTextCheckingResult *result=ary[i];
                        NSString *sonStr = [model.text substringWithRange:result.range];
                        [self.phoneAry addObject:sonStr];
                        //在某个范围内增加下划线
                        [atString addAttribute:NSUnderlineStyleAttributeName value:[NSNumber numberWithInteger:NSUnderlineStyleSingle] range:result.range];
                        //修改某个范围内字的颜色
                        [atString addAttribute:NSForegroundColorAttributeName value:
                         [UIColor blueColor] range:result.range];
                ...
                _bubbleView.textLabel.attributedText = atString;
            }
                break;
            case EMMessageBodyTypeImage:
            {
...
- (void)bubbleViewTapAction:(UITapGestureRecognizer *)tapRecognizer
{
    if (tapRecognizer.state == UIGestureRecognizerStateEnded) {
        ...
        switch (_model.bodyType) {
            case EMMessageBodyTypeText:
            {
                if (self.phoneAry.count<=0) {
                    return;
                }
                if ([_delegate respondsToSelector:@selector(callAction:)]) {
                    [_delegate callAction:self.phoneAry];
                }
            }
                break;
...
-(NSMutableArray *)getPhoneNumber:(NSString *)string
{
    ...
    [regex enumerateMatchesInString:string options:NSMatchingReportProgress range:NSMakeRange(0, string.length) usingBlock:^(NSTextCheckingResult * _Nullable result, NSMatchingFlags flags, BOOL * _Nonnull stop) {
        if (NSMatchingReportProgress==flags) {
            ...
            if (NSTextCheckingTypePhoneNumber==result.resultType) {
                [nsmuArray addObject:result];
//                NSLog(@"%@",[string substringWithRange:result.range]);
            } /** * 长度为11位的数字串 */
            if (result.range.length>=7&&result.range.length<=15) {
                [nsmuArray addObject:result];
//                NSLog(@"%@",[string substringWithRange:result.range]);
            }
        }
