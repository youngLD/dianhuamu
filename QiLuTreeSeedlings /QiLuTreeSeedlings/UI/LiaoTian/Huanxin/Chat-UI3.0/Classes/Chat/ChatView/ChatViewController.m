...
- (void)viewDidLoad {
    ...
    self.navigationController.navigationBar.tintColor =NavSColor;
...
- (void)_setupBarButtonItem
{
    ...
    if (self.conversation.type == EMConversationTypeChat) {
        ...
        [detailButton setImage:[UIImage imageNamed:@"details_norma"] forState:UIControlStateNormal];
   ...
   didSelectAvatarMessageModel:(id<IMessageModel>)messageModel
{
    ...
    YLDPersonMessageViewController *personMessageVC=[[YLDPersonMessageViewController alloc]initWithFrom:type withDic:messageModel.message.ext];
    personMessageVC.senderID=messageModel.message.from;
      personMessageVC.isSender=messageModel.isSender;
               ...
               selectAtTarget:(EaseSelectAtTargetCallback)selectedCallback
{
    ...
    if (chatGroup) {
        if (!chatGroup.occupants) {
            ...
            [self showHudInView:self.view hint:@"加载群成员"];
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                EMError *error = nil;
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (strongSelf) {
                        [strongSelf hideHud];
                        if (error) {
                            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"错误" message:[NSString stringWithFormat:@"获取群成员失败 [%@]", error.errorDescription] delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                            [alertView show];
                        }
                        else {
                           ...
                           modelForMessage:(EMMessage *)message
{
    ...
    model.failImageName = @"EaseUIResource.bundle/user";
    if (model.isSender) {
        
          model.avatarURLPath = APPDELEGATE.userModel.headUrl;
          model.nickname = [self getUserNick];
        
           } else {
            model.avatarURLPath = model.message.ext[@"avatar"];
               //昵称
            NSString *nickName=model.message.ext[@"nick"];
            
            if(nickName) {
                model.nickname = nickName;
            }else{
                NSDictionary *dic=[self searchWithStr:model.nickname];
                if (dic) {
                    model.avatarURLPath = dic[@"pic"];
                    model.nickname=dic[@"nickName"];
                    NSMutableDictionary *ext=[NSMutableDictionary dictionary];
                    ext[@"nick"]=dic[@"nickName"];
                    ext[@"avatar"]=dic[@"pic"];
                    message.ext=ext;
...
-(NSDictionary *)searchWithStr:(NSString *)str
{
    NSString *phone =[NSString stringWithFormat:@"member CONTAINS[cd] '%@'",str];
    //    workstationName
    NSPredicate *phonePredicate = [NSPredicate predicateWithFormat:phone];
    NSArray *arrayPre=[self.personAry filteredArrayUsingPredicate:phonePredicate];
    NSDictionary *dic=[arrayPre firstObject];
    return dic;
}
-(NSString *)getUserNick
{
    NSString *str = nil;
    switch (APPDELEGATE.userModel.goldsupplierStatus) {
        case 1:
            str=@"金-";
            break;
        case 2:
    ...
    NSString *nickStr=[NSString stringWithFormat:@"%@%@",str,APPDELEGATE.userModel.name];
    return nickStr;
}
                    ...
                    messageType:(EMMessageBodyType)messageType
{
    ...
    if (_transpondMenuItem == nil) {
        _transpondMenuItem = [[UIMenuItem alloc] initWithTitle:@"粘贴" action:@selector(transpondMenuAction:)];
    }
