//
//  ZIKMyOfferQuotationTableViewCell.m
//  QiLuTreeSeedlings
//
...
@interface ZIKMyOfferQuotationTableViewCell ()
...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *itemTrailingLayoutConstraint;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *itemNameHeightLayoutConstraint;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *itemTopLayoutConstraint;

@end
@implementation ZIKMyOfferQuotationTableViewCell
...
+ (instancetype)cellWithTableView:(UITableView *)tableView {
    ...
    ZIKMyOfferQuotationTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:kdzZIKMyQuotationTableViewCellID];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"ZIKMyOfferQuotationTableViewCell" owner:self options:nil] lastObject];
        cell.bottomBgImageView.image = [ZIKMyOfferQuotationTableViewCell imageWithSize:cell.bottomBgImageView.frame.size borderColor:NavColor borderWidth:1];
    }
    return cell;
}
...
- (void)configureCell:(ZIKMyOfferQuoteListModel *)model {
    ...
    CGRect rect =  [model.itemName boundingRectWithSize:CGSizeMake(kWidth/2-30, CGFLOAT_MAX)
                                             ...
                                             attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15.0f]}
                                                context:nil];
    self.itemNameHeightLayoutConstraint.constant = rect.size.height+5;
//    if (rect.size.width+8+CGRectGetMaxX(self.itemNameLabel.frame)>=CGRectGetMaxX(_itemQuantityLabel.frame)) {
//        self.itemTrailingLayoutConstraint.constant = 5;
//    }
//    else {
    self.itemNameLabelLayoutConstraint.constant = rect.size.width+10;
//    }
