...
//    _deleteButton.hidden=YES;
    ...
    NSArray *views = @[_iconView, _nameLable, _contentLabel, _moreButton, _picContainerView, _timeLabel, _deleteButton, _operationButton, _operationMenu, _commentView];
...
- (void)setModel:(SDTimeLineCellModel *)model
{
    ...
    [_iconView setImageWithURL:[NSURL URLWithString:model.iconName] placeholderImage:[UIImage imageNamed:@"qiugouxiangqingye"]];
    ...
    _timeLabel.text = model.timec;
    
    if (model.memberUid.length>0) {
        if ([model.memberUid isEqualToString:APPDELEGATE.userModel.access_id]) {
...
{
    if ([self.delegate respondsToSelector:@selector(deleteClickActionWithModel:)]) {
        [self.delegate deleteClickActionWithModel:self.model];
    }
