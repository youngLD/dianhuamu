...
- (NSMutableAttributedString *)generateAttributedStringWithCommentItemModel:(SDTimeLineCellCommentItemModel *)model
{
    ...
    [attString addAttribute:NSLinkAttributeName value:model.firstUserId range:[text rangeOfString:model.firstUserName]];
//    [attString setAttributes:@{NSLinkAttributeName : model.firstUserId} range:];
    if (model.secondUserName) {
            [attString addAttribute:NSLinkAttributeName value:model.secondUserId range:[text rangeOfString:model.secondUserName]];
//        [attString setAttributes:@{NSLinkAttributeName : model.secondUserId} range:];
    }
    return attString;
}
...
- (void)didClickLink:(MLLink *)link linkText:(NSString *)linkText linkLabel:(MLLinkLabel *)linkLabel
{
    if (link.linkType==MLLinkTypeOther) {
        if (self.didClickCommentLabelBlock) {
            ...
            self.didClickCommentLabelBlock(linkText,rect);
        }
...
-(void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    int y = point.y;
    CGPoint pointloct=CGPointMake(30, y);
    for (UILabel *lab in self.commentLabelsArray) {
        if (CGRectContainsPoint(lab.frame,pointloct)) {
            
            SDTimeLineCellCommentItemModel *model=self.commentItemsArray[lab.tag];
            if (self.didClickCommentLabelBlock) {
                UIWindow *window = [UIApplication sharedApplication].keyWindow;
                CGRect rect = [self.superview convertRect:self.frame toView:window];
...
- (void)didtouchblankAreaAtion:(NSInteger)index
{
