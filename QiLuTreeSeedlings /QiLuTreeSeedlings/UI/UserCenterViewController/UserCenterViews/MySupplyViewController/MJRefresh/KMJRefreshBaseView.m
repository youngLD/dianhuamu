...
@implementation KMJRefreshBaseView
#pragma mark - 控件初始化
/**
 *  状态标签
 */
...
{
    if (!_arrowImage) {
        UIImageView *arrowImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:KMJRefreshSrcName(@"arrow.png")]];
...
- (instancetype)initWithFrame:(CGRect)frame {
    frame.size.height = KMJRefreshViewHeight;
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    ...
    // 旧的父控件
    [self.superview removeObserver:self forKeyPath:KMJRefreshContentOffset context:nil];
    
    if (newSuperview) { // 新的父控件
        [newSuperview addObserver:self forKeyPath:KMJRefreshContentOffset options:NSKeyValueObservingOptionNew context:nil];
...
- (void)setState:(MJRefreshState)state
{
    ...
    switch (state) {
        ...
        {
            if (self.state == MJRefreshStateRefreshing) {
                [UIView animateWithDuration:KMJRefreshSlowAnimationDuration * 0.6 animations:^{
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(KMJRefreshSlowAnimationDuration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    // 再次设置回normal
