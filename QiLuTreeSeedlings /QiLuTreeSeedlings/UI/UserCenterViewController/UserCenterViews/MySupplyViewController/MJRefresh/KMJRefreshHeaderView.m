...
@implementation KMJRefreshHeaderView
#pragma mark - 控件初始化
/**
 *  时间标签
 */
- (UILabel *)lastUpdateTimeLabel
{
    if (!_lastUpdateTimeLabel) {
        ...
        // 2.加载时间
        self.lastUpdateTime = [[NSUserDefaults standardUserDefaults] objectForKey:KMJRefreshHeaderTimeKey];
    }
...
+ (instancetype)header
{
    return [[KMJRefreshHeaderView alloc] init];
}

- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        self.pullToRefreshText = KMJRefreshHeaderPullToRefresh;
        self.releaseToRefreshText = KMJRefreshHeaderReleaseToRefresh;
        self.refreshingText = KMJRefreshHeaderRefreshing;
    }
    return self;
}
...
- (void)setLastUpdateTime:(NSDate *)lastUpdateTime
{
    ...
    // 1.归档
    [[NSUserDefaults standardUserDefaults] setObject:lastUpdateTime forKey:KMJRefreshHeaderTimeKey];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([KMJRefreshContentOffset isEqualToString:keyPath]) {
