...
#import "KMJRefreshFooterView.h"
#import "KMJRefreshConst.h"
#import "UIView+KMJExtension.h"
#import "UIScrollView+KMJExtension.h"

@interface KMJRefreshFooterView()
@property (assign, nonatomic) int lastRefreshCount;
@end

@implementation KMJRefreshFooterView

+ (instancetype)footer
{
    return [[KMJRefreshFooterView alloc] init];
}

- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        self.pullToRefreshText = KMJRefreshFooterPullToRefresh;
        self.releaseToRefreshText = KMJRefreshFooterReleaseToRefresh;
        self.refreshingText = KMJRefreshFooterRefreshing;
    }
    return self;
}
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    ...
    // 旧的父控件
    [self.superview removeObserver:self forKeyPath:KMJRefreshContentSize context:nil];
    
    if (newSuperview) { // 新的父控件
        // 监听
        [newSuperview addObserver:self forKeyPath:KMJRefreshContentSize options:NSKeyValueObservingOptionNew context:nil];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([KMJRefreshContentSize isEqualToString:keyPath]) {
        // 调整frame
        [self adjustFrameWithContentSize];
    } else if ([KMJRefreshContentOffset isEqualToString:keyPath]) {
	...
	switch (state)
    {
		case MJRefreshStateNormal:
        {
            ...
            if (MJRefreshStateRefreshing == oldState) {
                ...
                [UIView animateWithDuration:KMJRefreshSlowAnimationDuration animations:^{
                    self.scrollView.mj_contentInsetBottom = self.scrollViewOriginalInset.bottom;
                }];
            } else {
                // 执行动画
                [UIView animateWithDuration:KMJRefreshFastAnimationDuration animations:^{
                    self.arrowImage.transform = CGAffineTransformMakeRotation(M_PI);
                }];
            }
